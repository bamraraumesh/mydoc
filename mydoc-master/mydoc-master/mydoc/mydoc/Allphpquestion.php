...
questio : what is anonymous function or lambda function ?
An anonymous function, also known as a lambda function or a closure, is a function in PHP that doesn't have a specified name and can be defined inline wherever it's needed. Unlike regular functions, anonymous functions can be created dynamically at runtime and assigned to variables or passed as arguments to other functions.

Here's the basic syntax of an anonymous function in PHP:
...
Anonymous functions are useful in situations where you need to define a simple, short-lived function without the overhead of declaring a named function. They are commonly used as callbacks in functions like array_map(), array_filter(), and usort(), or for implementing callback-style APIs.

Here's an example of using an anonymous function as a callback with array_map():
...
In this example, the anonymous function (function($num) { return $num * $num; }) squares each number in the $numbers array when passed as a callback to array_map().

Anonymous functions support lexical scoping, meaning they can access variables from the parent scope. This makes them particularly useful for implementing closures, which are functions that capture variables from their surrounding context.

Overall, anonymous functions provide a concise and flexible way to define small, reusable blocks of code inline within PHP applications.